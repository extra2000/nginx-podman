...
Instructions how to prepare a fresh installed Almalinux 8 for deploying NGINX.
...
Enable Unified Cgroup. In ``/etc/default/grub`` file, add ``systemd.unified_cgroup_hierarchy=1`` into ``GRUB_CMDLINE_LINUX``. Then, update GRUB:
